...
public class TsubuyakiController {
    ...
    String showTsubuyakiList(Model model) {
        ...
        model.addAttribute("keywordForm", new KeywordForm());  //空フォームをセット
    ...
    @GetMapping("/search")
    String showSearch(@ModelAttribute("keywordForm") KeywordForm form, Model model) {
        List<Tsubuyaki> searchResultList = ts.getSearchResults(form.getKeyword()); //検索結果を取得
        model.addAttribute("searchResultNum", searchResultList.size()); //検索結果数をモデルにセット
        model.addAttribute("searchResultList", searchResultList); //モデル属性にリストをセット
